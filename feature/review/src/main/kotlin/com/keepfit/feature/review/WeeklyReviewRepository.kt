...
class RoomWeeklyReviewRepository @Inject constructor(
    ...
    override suspend fun load(): WeeklyReviewSnapshot {
        ...
        val schedule = workoutDao.observeWeeklySchedule().first()
        val occurrences = workoutDao.observeOccurrenceDetails(window.reviewStart, window.reviewEnd).first()
        val sessions = workoutDao.observeSessionsBetween(window.comparisonStart.minusDays(14), window.reviewEnd).first()
        ...
        val nutrition = nutritionSignal(window, settings.nutritionTrackingDepth)
        val currentPerformance = workoutDao.findExercisePerformanceBetween(window.reviewStart, window.reviewEnd)
            .associateBy { it.exerciseId }
        val comparisonPerformance = workoutDao.findExercisePerformanceBetween(window.comparisonStart, window.comparisonEnd)
            .associateBy { it.exerciseId }
        ...
        val outcome = database.weeklyReviewDao().findForWeek(window.reviewStart)
        return when {
    ...
    override suspend fun approve(draft: ReviewDraft) {
        ...
        database.withTransaction {
            database.weeklyReviewDao().findForWeek(window.reviewStart)?.let { return@withTransaction }
            val planned = requireNotNull(workoutDao.findPlannedWorkout(draft.sourcePlannedWorkoutId)) {
            ...
            require(workoutDao.findOccurrenceForSource(planned.id, draft.sourceDate) == null) {
                "That workout already has a one-week adjustment."
            }
            val template = requireNotNull(workoutDao.findTemplateDetails(planned.workoutTemplateId)) {
    ...
    override suspend fun dismiss() {
        val window = rules.windowFor(today())
        database.withTransaction {
            if (database.weeklyReviewDao().findForWeek(window.reviewStart) != null) return@withTransaction
            database.weeklyReviewDao().upsert(
                WeeklyReviewOutcomeEntity(
                    id = idFactory(),
                    weekStart = window.reviewStart,
    ...
    private suspend fun nutritionSignal(
        ...
        if (depth == NutritionTrackingDepth.MEAL_QUALITY) {
            val current = nutritionDao.getMealQualityCheckIns(window.reviewStart, window.reviewEnd)
            val prior = nutritionDao.getMealQualityCheckIns(window.comparisonStart, window.comparisonEnd)
        ...
        val totals = nutritionDao.observeAllDailyTotals().first()
    ...
    private suspend fun firstExerciseIsAtSetCap(draft: ReviewDraft): Boolean {
        val planned = workoutDao.findPlannedWorkout(draft.sourcePlannedWorkoutId) ?: return true
        val firstExercise = workoutDao.findTemplateDetails(planned.workoutTemplateId)
            ?.exercises
